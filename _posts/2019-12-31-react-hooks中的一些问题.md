...
>1. 问题一：函数组件中，初始化的函数方法拿不到最新的状态值(已解决)

>2. 问题二，在函数组件中，给antd的级联选择器初始化options的时候，改变options状态值，但是级联选择器中的数据依旧是初始值。(未解决)

>3. 问题三，在函数组件中，父组件如何调用子组件中的方法。(已解决)
   
>4. 问题四，在被Form.create()()包装过后的子组件，父组件无法直接传递值到子组件中去,同时也无法调用子组件中的方法(已解决)
